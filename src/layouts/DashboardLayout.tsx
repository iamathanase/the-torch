...
export default function DashboardLayout() {
  ...
  return (
    <div className="flex min-h-screen w-full bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50 dark:bg-slate-950">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex lg:flex-col lg:w-64 border-r border-emerald-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex-shrink-0">
        <div className="border-b border-emerald-200 dark:border-slate-700 py-4 bg-gradient-to-r from-emerald-500 to-green-600">
          <NavLink to="/" className="flex items-center gap-2.5 px-4">
            ...
            <span className="font-display text-lg font-bold text-white">The Torch</span>
          </NavLink>
        </div>
        
        <div className="flex-1 overflow-y-auto py-4">
          <div className="px-4 py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Workspace</div>
          <nav className="space-y-1 px-2 mt-2">
            {items.map((it) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all font-medium w-full ${isActive ? "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 shadow-sm" : "text-slate-600 dark:text-slate-400 hover:bg-emerald-50 dark:hover:bg-slate-800 hover:text-emerald-600 dark:hover:text-emerald-400"}`
                }
              >
        ...
        <div className="border-t border-emerald-200 dark:border-slate-700 p-4 bg-emerald-50 dark:bg-slate-900">
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleLogout} className="rounded-lg p-2 text-slate-600 dark:text-slate-300 hover:bg-red-100 dark:hover:bg-red-900/30 hover:text-red-600 transition-all" aria-label="Sign out">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed left-0 top-0 bottom-0 w-72 bg-white dark:bg-slate-900 shadow-xl flex flex-col">
            <div className="border-b border-emerald-200 dark:border-slate-700 py-4 bg-gradient-to-r from-emerald-500 to-green-600">
              <div className="flex items-center justify-between px-4">
                ...
                <button onClick={() => setSidebarOpen(false)} className="text-white hover:bg-white/20 rounded-lg p-1.5">
                  <X className="h-5 w-5" />
                </button>
              </div>
            </div>
